...
| `altllm-portal-api-keys` | `list-api-keys`, `create-api-key`, `get-api-key`, `update-api-key`, `revoke-api-key` | Keys are for the OpenAI-compatible gateway, not Portal API auth |
| `altllm-portal-billing` | `credit`, `redeem-promo`, `transactions`, `usage-summary`, `usage-timeline`, `usage-by-model`, `usage-by-key` | Mostly raw Portal API JSON responses |
| `altllm-portal-payments` | `topup-crypto`, `payment-status`, `pay-payment-link` | Includes direct-payment guardrails and supported pay currencies |
