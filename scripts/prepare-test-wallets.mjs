...
async function main() {
  ...
  const loginLines = [
    ...
    `WALLETS_FILE=${shellQuote(`${outDir}/wallets.public.json`)}`,
    "",
    "# This uses the CLI external-signature path: prepare challenge, sign locally, then verify.",
    'node scripts/login-test-wallets.mjs --base-url "$BASE_URL" --wallets "$WALLETS_FILE"',
    "",
  ];
